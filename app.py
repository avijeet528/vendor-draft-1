...
def extract_hyperlinks(file_path):
    ...
    try:
        ...
        st.warning(f"Hyperlink extraction warning: {e}")
    return link_map
...
def load_data():
    ...
    for i, row in raw.iterrows():
        ...
        if (any("category" in v for v in rv) and
                any("file" in v for v in rv)):
            hdr = i
            break
    if hdr is None:
...
with st.sidebar:
    ...
    count_label = f"({len(avail_svcs)} available)"
    st.markdown(
        "<p style='color:#ECECEC;font-weight:700;font-size:0.8em;"
        "text-transform:uppercase;letter-spacing:0.06em;"
        f"margin:12px 0 4px'>🛠 Select Services "
        f"<span style='font-weight:400;color:#7D7D7D'>"
        f"{count_label}</span></p>",
        unsafe_allow_html=True)
...
if not pivot.empty:
    fig_stacked = px.bar(
        ...
        title="Services Offered per Vendor — by Category",
...
with c_left:
    ...
    fig_shared.add_annotation(
        ...
        showarrow=False,
        font=dict(size=9, color=PWC_GREY),
        xanchor="right")
...
if not selected_svcs:
    st.markdown(
        ...
        f"See full vendor &amp; pricing breakdown."
        f"</span></div>",
        unsafe_allow_html=True)
else:
    ...
    if d_sel.empty:
        st.warning("⚠️ No results for selected service(s).")
    else:
            ...
            with st.expander(
                ...
                for bi, (_, brow) in enumerate(d_svc.iterrows()):
                    ...
                    if burl and burl.startswith("http"):
                        ...
                        with btn_cols[cidx]:
                            st.button(
                                f"🚫 {blabel}",
                                key=f"na_{bkey}_{bi}_{svc[:10]}",
                                disabled=True,
                                help="No URL available")
                ...
                if ext_results:
                    ...
                    sum_tbl = [
                        ...
                        "<th style='width:18%'>Line Items Found</th>"
                        "<th style='width:24%'>Extracted Total</th>"
                        "</tr></thead><tbody>"
                    ]
                    running = 0.0
                    for er in ext_results:
                        ...
                        sum_tbl.append(
                            ...
                            f"<td style='font-family:monospace;"
                            f"font-size:0.80em;color:{PWC_DARK};"
                            f"word-break:break-all'>{fn_s}</td>"
